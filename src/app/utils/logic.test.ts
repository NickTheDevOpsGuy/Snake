...
describe("logic", () => {
  ...
  describe("inferDirFromSnake", () => {
    it("infers direction from head and second segment", () => {
      expect(inferDirFromSnake([{ x: 2, y: 1 }, { x: 1, y: 1 }])).toBe("right");
      expect(inferDirFromSnake([{ x: 1, y: 1 }, { x: 2, y: 1 }])).toBe("left");
      expect(inferDirFromSnake([{ x: 1, y: 0 }, { x: 1, y: 1 }])).toBe("up");
      expect(inferDirFromSnake([{ x: 1, y: 1 }, { x: 1, y: 0 }])).toBe("down");
    });
